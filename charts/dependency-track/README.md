...
| fileStorage.s3.credentials.existingSecret | object | `{"accessKeyIdKey":"accessKeyId","name":"","secretAccessKeyKey":"secretAccessKey"}` | Reference an existing Secret. Credentials are mounted as files under `/etc/dt/secrets/s3/`. Ignored when `authentication.type` is `auto`. |
