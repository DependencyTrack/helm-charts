...
| ingress.tls | list | `[]` |  |
| nameOverride | string | `""` |  |
| secretManagement.database.kek | object | `{"existingSecret":{"key":"","name":""},"mode":"config"}` | The key encryption key (KEK) configuration. Required when `provider: database`. The chart does not generate the KEK. Provision it separately and reference it here. |
