...
![Version: 1.2.0](https://img.shields.io/badge/Version-1.2.0-informational?style=flat-square) ![Type: application](https://img.shields.io/badge/Type-application-informational?style=flat-square) ![AppVersion: 4.14.2](https://img.shields.io/badge/AppVersion-4.14.2-informational?style=flat-square)
...
| apiServer.probes.liveness.path | string | `"/health/live"` |  |
...
| apiServer.probes.readiness.path | string | `"/health/ready"` |  |
...
| apiServer.probes.startup.path | string | `"/health/started"` |  |
...
| apiServer.replicaCount | int | `1` | The number of replicas to deploy |
...
| ingress.tls | list | `[]` |  |
| migration.enabled | bool | `false` | Whether to enable the migration job (see [MIGRATION.md](MIGRATION.md)) |
| migration.image.pullPolicy | string | `"IfNotPresent"` |  |
| migration.image.registry | string | `"ghcr.io"` |  |
| migration.image.repository | string | `"dependencytrack/v4-migrator"` |  |
| migration.image.tag | string | `"5-snapshot"` |  |
| migration.resources.limits.cpu | int | `1` |  |
...
| migration.sourceDatabase.jdbcUrl | string | `"jdbc:postgresql://{{ .Release.Name }}-postgres:5432/owasp-dt"` | Source database URL |
| migration.targetDatabase.existingSecret.name | string | `"{{ .Release.Name }}-database-secret"` | Name of the existing secret |
| migration.targetDatabase.existingSecret.passwordKey | string | `"password"` | Key in the secret that's holding the password for target database |
| migration.targetDatabase.existingSecret.usernameKey | string | `"username"` | Key in the secret that's holding the username for target database |
| migration.targetDatabase.jdbcUrl | string | `"jdbc:postgresql://{{ .Release.Name }}-postgres-v5:5432/owasp-dt"` | Target database URL |
